    ...
    for i, custom in ipairs(custom_callouts) do
      ...
      if (el.classes:includes(custom_class)) then
        ...
        if custom.color then
          callout.attributes['color']=pandoc.utils.stringify(custom.color)
        end
        ...
        if quarto.doc.is_format('docx') then
          ...
          if callout.attributes == nil then
            callout.attributes = pandoc.Attr({ ["custom-style"] = stylename })
          else
            callout.attributes["custom-style"] = stylename
          end
          quarto.log.info('Attribs:')
          quarto.log.info(callout.attributes)
        end
        return callout
      end
    end
